    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h4>{{ucfirst($title)}} {{__('Kriteria')}} {{$subkriteria->nama}}</h4>
            </div>
            <div class="card-body">
                <a href="{{ route('kategorisubkriteria.index', $subkriteria->id) }}" class="btn btn-sm btn-danger mb-3"
                    role="button">
                    <i class="fa fa-arrow-left"></i>
                    {{__('Kembali')}}
                </a>
                <form action="{{ route('bobotkategorisubkriteria.index', $subkriteria->id) }}" method="post">
                    @csrf
